...
"""),
        md("## 4. Experiments\n\nEach research question varies exactly one axis with "
           ...
           "This runs the **complete study** end-to-end. Pick a `PRESET`:\n\n"
           "* `smoke` — proves the whole pipeline in minutes (tiny scope);\n"
           "* `kaggle` — real trends, 3 seeds (fits one ~12h CPU session);\n"
           "* `kaggle8` — the publication scope, **8 seeds** (RQ5 self-caps to 5) — this is\n"
           "  what the paper reports; it exceeds one 12h Kaggle session, so run it on a\n"
           "  machine without that limit, or let it resume (the driver checkpoints every\n"
           "  cell and skips finished RQs, so re-running continues where it stopped);\n"
           "* `full` — 250 images/cell, 8 seeds (largest)."),
        code('''# Run the complete study: every RQ + the ablations, in order.
PRESET = "kaggle8"        # smoke | kaggle | kaggle8 (publication) | full
JOBS   = 4                # parallel cells; set to the session's CPU count

import subprocess, sys
RQS = ["rq1", "rq2", "rq3", "rq4", "rq5",
       "ablation_encoding", "ablation_depth", "ablation_dataset"]
for rq in RQS:
    print(f"\\n########## {rq} ##########", flush=True)
